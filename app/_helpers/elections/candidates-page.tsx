...
export function CandidatesHero({
  ...
  election: CandidatesElectionMeta;
}) {
  return (
    <div className="text-center space-y-2 pt-3">
      ...
      <div className="flex items-center justify-center gap-6 text-sm text-muted-foreground font-medium">
        <div className="flex items-center gap-2">
          ...
          <span>{formatDisplayDate(election.start_date)}</span>
        </div>
        <div className="h-4 w-px bg-border" />
        <div className="flex items-center gap-2">
          <Clock3 className="size-4" />
          <span>
            {formatDisplayTimeRange(election.start_date, election.end_date)}
          </span>
        </div>
      </div>
    </div>
...
export function CandidateSearchBar({ query }: { query: string }) {
  return (
...
export function normalizeCandidates(
  candidates: CandidatesRaw[] | null | undefined,
) {
  return (candidates || []).map((candidate) => {
    ...
    return {
      ...
      partylists: party,
    } satisfies CandidatesRow;
  });
}

export function filterCandidates(candidates: CandidatesRow[], query: string) {
...
export function buildCandidatesByPosition(candidates: CandidatesRow[]) {
...
export function CandidatePositionSection({
  position,
  candidates,
  electionId,
}: {
  position: CandidatesPositionRow;
  candidates: CandidatesRow[];
  electionId: string;
}) {
  ...
  return (
    <section className="space-y-4">
      <h3 className="text-4xl font-black tracking-tight">{position.title}</h3>
            ...
            >
              <CardContent className="p-4 flex flex-col items-center text-center gap-3">
                ...
                <Button asChild className="w-full font-semibold" size="sm">
                  <Link href={`/elections/${electionId}/manage-partylist`}>
                    View Platform
                  </Link>
                </Button>
              </CardContent>
...
export async function CandidatesPageContent({
  electionId,
  query,
}: CandidatesContentProps) {
  ...
  const candidatesNormalized = normalizeCandidates(
    candidates as CandidatesRaw[],
  );
  const filtered = filterCandidates(candidatesNormalized, query);
  const positionMap = buildCandidatesByPosition(filtered);

  return (
    <main className="min-h-screen bg-background">
      <div className="container max-w-7xl mx-auto px-4 py-4 space-y-6">
        <CandidatesHero election={electionMeta} />

        <div className="flex items-end justify-between gap-6">
          <h2 className="text-4xl font-black tracking-tight text-foreground">
            Candidate Directory
          </h2>
          <CandidateSearchBar query={query} />
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-8">
            {((positions || []) as CandidatesPositionRow[]).map((position) => (
              <CandidatePositionSection
                key={position.position_id}
                position={position}
                candidates={positionMap.get(position.position_id) || []}
