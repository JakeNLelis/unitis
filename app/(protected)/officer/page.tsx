...
import { ArrowRight, PlusCircle, Users, Vote, ClipboardList, Activity } from "lucide-react";
...
export default async function OfficerDashboardPage() {
  ...
  const [
    ...
    supabase.from("candidates").select("*", { count: "exact", head: true }).eq("application_status", "pending"),
  ...
  const { data: recentApplications } = await supabase
    .from("candidates")
    .select("candidate_id, full_name, created_at, application_status, elections(name)")
    ...
    <div className="space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
          <div className="space-y-2">
            ...
            <p className="text-xl font-medium max-w-xl border-l-4 border-primary pl-4 py-2 bg-surface-low">
              Welcome back, <span className="font-bold text-primary">{profile.display_name}</span>. 
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border border-b border-border">
        <div className="p-8 bg-background hover:bg-surface-low transition-colors">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-muted-foreground">Registry</span>
          </div>
          <div className="text-5xl font-black font-heading mb-1">{totalElections}</div>
          <p className="text-sm font-bold uppercase tracking-tight text-muted-foreground">Total Elections</p>
        </div>

        <div className="p-8 bg-background hover:bg-surface-low transition-colors">
          <div className="flex items-center justify-between mb-4">
            <Activity className="size-6 text-destructive" />
            <span className="text-xs font-black uppercase tracking-widest text-destructive">Urgent</span>
          </div>
          <div className="text-5xl font-black font-heading mb-1">{pendingCandidates}</div>
          <p className="text-sm font-bold uppercase tracking-tight text-muted-foreground">Pending Applications</p>
        </div>
      </section>

      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black font-heading uppercase tracking-tighter">Application Audit Trail</h2>
          <Link href="/officer/elections" className="text-sm font-bold underline hover:text-primary">Full Registry {">"}</Link>
        </div>

        <div className="border-2 border-foreground overflow-hidden">
          <div className="divide-y-2 divide-foreground">
            {recentApplications?.map((app) => (
              <div key={app.candidate_id} className="group hover:bg-surface-low transition-colors">
                <InstitutionalListItem
                  title={app.full_name}
                  subtitle={(app.elections as any)?.name || "Unknown Election"}
            ...
            {(!recentApplications || recentApplications.length === 0) && (
              <div className="p-12 text-center bg-surface-low">
                <p className="font-bold text-muted-foreground uppercase">No recent activity detected in the audit log.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-8 border-2 border-foreground bg-surface-low group hover:bg-primary transition-all cursor-pointer">
          <h3 className="text-xl font-bold uppercase tracking-tight group-hover:text-white mb-2">Voter Registry</h3>
          <p className="text-sm text-muted-foreground group-hover:text-white/80 mb-6">Manage the global list of eligible student voters across all faculties.</p>
        ...
        <div className="p-8 border-2 border-foreground bg-surface-low group hover:bg-foreground transition-all cursor-pointer">
          <h3 className="text-xl font-bold uppercase tracking-tight group-hover:text-white mb-2">System Reports</h3>
          <p className="text-sm text-muted-foreground group-hover:text-white/80 mb-6">Generate official election certificates and turnout documentation.</p>
          <div className="flex items-center text-sm font-black uppercase group-hover:text-white">
